...
The three home feature images are generated by driving the real app against fixture data, then post-processed into hashed WebP under `src/assets/`. Nothing runs this automatically, and nothing detects that they have gone stale — so if you change the **print list**, the **materials list**, or the **analytics overview tab**, refresh them in the same PR:

```bash
# Boots the dev server, captures, then writes the hashed WebP and updates home.component.html
npm run capture:home:all

# If a dev server is already on 4200, run the two steps directly
npm run capture:home
npm run capture:home:process
```

Commit the resulting images along with `src/app/home/home.component.html`. Chrome is required — the 2× device-scale-factor flag has no effect in Electron, and the processing step rejects a 1× capture rather than publish a half-resolution image. [cypress/CLAUDE.md](cypress/CLAUDE.md) documents the fixtures, the offline guard, and the pitfalls.
