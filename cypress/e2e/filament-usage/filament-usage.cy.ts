describe('Filament Usage', () => {
  ...
  it('should track remaining weight when filament is used in a print', () => {
    const filamentName = 'Weight Test Filament - ' + new Date().getTime();
    const printTitle = 'Weight Test Print - ' + new Date().getTime();

    // Create a filament with 1250g total weight and 1000g nominal weight
    cy.visit('/filament');
    ...
    // Create a new print (title + printer required; date/time default to now)
    ...
    // After save the app navigates to /prints — click into the new print
    cy.contains('[cy-print-row]', printTitle).click();

    // Edit the print: add filament usage of 100g
    ...
    // Verify remaining weight on filament list: 1000g - 100g = 900g
  ...
  it('should allow adding multiple filament records to a print', () => {
    ...
    // Edit: add first filament usage (pick first filament from modal)
    cy.get('button[data-cy-edit-btn]').click();
    cy.get('#add-new-filament-usage-btn').click();
    ...
    // Add second filament usage record
    cy.get('#add-new-filament-usage-btn').click();
    ...
    // After save navigate back to the print and re-enter edit mode
    cy.contains('[cy-print-row]', printTitle).click();
  ...
  it('should allow editing filament usage amount', () => {
    ...
    // Edit: add filament usage of 50g
    cy.get('button[data-cy-edit-btn]').click();
    cy.get('#add-new-filament-usage-btn').click();
    cy.intercept('GET', '/api/Filaments*').as('getFilamentsModal');
    cy.get('[data-cy="select-filament-btn"]').click();
    ...
    // Navigate back to the print, edit again: change the amount from 50g to 75g
    ...
    // Navigate back and verify the updated amount persisted
    cy.contains('[cy-print-row]', printTitle).find('.mat-column-title').click();
    cy.get('button[data-cy-edit-btn]').should('be.visible').click();
  ...
  it('should allow removing filament usage from a print', () => {
    ...
    // Navigate back, delete the filament usage record
    cy.contains('[cy-print-row]', printTitle).find('.mat-column-title').click();
    cy.get('button[data-cy-edit-btn]').should('be.visible').click();
    ...
    // Navigate back and verify no filament entry cards remain
    cy.contains('[cy-print-row]', printTitle).find('.mat-column-title').click();
    cy.get('button[data-cy-edit-btn]').should('be.visible').click();
    cy.get('.filament-entry-card').should('not.exist');
  });
